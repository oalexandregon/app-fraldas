...
const Eat = ({ data, setData, translate }) => {
  ...
  return <Grid container={true} spacing={2} sx={{ display: "flex", justifyContent: 'center' }}>
    <Grid item={true} size={{ xs: 12 }} sx={{ width: "clamp(100px, 80vw, 500px)" }}>
      ...
      <Button variant='contained' color={data.type === 2 ? "secondary" : "primary"} onClick={() => {
    ...
    {
      data.type === 2 ? <Grid item={true} size={{ xs: 12 }} sx={{ width: "clamp(100px, 80vw, 500px)" }}>
        <DateTimePicker
    ...
    {
      data.type === 1 ? <Grid item={true} size={{ xs: 12 }} sx={{ width: "clamp(100px, 80vw, 500px)" }}>
        <TextField
